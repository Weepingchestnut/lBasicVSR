...
def get_torchvision_models():
    if digit_version(torchvision.__version__) < digit_version('0.13.0a0'):
        ...
        json_path = osp.join(lmmcv.__path__[0],
                             'model_zoo/torchvision_0.12.json')
        model_urls = lmmcv.load(json_path)
...
def get_external_models():
    ...
    default_json_path = osp.join(lmmcv.__path__[0], 'model_zoo/open_mmlab.json')
...
def get_mmcls_models():
    mmcls_json_path = osp.join(lmmcv.__path__[0], 'model_zoo/mmcls.json')
...
def get_deprecated_model_names():
    deprecate_json_path = osp.join(lmmcv.__path__[0],
...
def save_checkpoint(model: torch.nn.Module,
    ...
    meta.update(lmmcv_version=lmmcv.__version__, time=time.asctime())
