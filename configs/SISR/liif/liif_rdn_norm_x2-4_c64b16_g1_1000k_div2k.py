...
train_pipeline = [
    dict(
        type='LoadImageFromFile',   # ndarray
        io_backend='disk',
        key='gt',
        flag='color',
        channel_order='rgb'),
    dict(
        type='RandomDownSampling',  # ndarray
        scale_min=scale_min,
        scale_max=scale_max,
        patch_size=48),
    dict(type='RescaleToZeroOne', keys=['lq', 'gt']),   # 归一化 ndarray
    dict(type='Flip', keys=['lq', 'gt'], flip_ratio=0.5, direction='horizontal'),   # ndarray
    dict(type='Flip', keys=['lq', 'gt'], flip_ratio=0.5, direction='vertical'),     # ndarray
    dict(type='RandomTransposeHW', keys=['lq', 'gt'], transpose_ratio=0.5),         # ndarray
    dict(type='ImageToTensor', keys=['lq', 'gt']),      # tensor
