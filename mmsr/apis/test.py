...
def single_gpu_test(model,
    ...
    prog_bar = lmmcv.ProgressBar(len(dataset))
...
def collect_results_cpu(result_part, size, tmpdir=None):
    ...
    if tmpdir is None:
        ...
        dir_tensor = torch.full((MAX_LEN, ),
                                ...
                                device='cuda')
        if rank == 0:
            lmmcv.mkdir_or_exist('.dist_test')
        ...
        lmmcv.mkdir_or_exist(tmpdir)
    # synchronizes all processes to make sure tmpdir exist
    dist.barrier()
    # dump the part result to the dir
    lmmcv.dump(result_part, osp.join(tmpdir, 'part_{}.pkl'.format(rank)))
    ...
    for i in range(world_size):
        ...
        part_list.append(lmmcv.load(part_file))
    # sort the results
    ordered_results = []
