...
class Crop:
    ...
    def __init__(self, keys, crop_size, random_crop=True, is_pad_zeros=False):
        if not lmmcv.is_tuple_of(crop_size, int):
            raise TypeError(
    ...
    def __init__(self, keys, crop_size, crop_pos=None):
        if not lmmcv.is_tuple_of(crop_size, int):
            raise TypeError(
                'Elements of crop_size must be int and crop_size must be'
                f' tuple, but got {type(crop_size[0])} in {type(crop_size)}')
        if not lmmcv.is_tuple_of(crop_pos, int) and (crop_pos is not None):
            raise TypeError(
...
class CropAroundCenter:
    ...
    def __init__(self, crop_size):
        if lmmcv.is_tuple_of(crop_size, int):
    ...
    def __call__(self, results):
        ...
        if rescale_ratio > 1:
            ...
            fg = lmmcv.imresize(fg, (new_w, new_h), interpolation='nearest')
            alpha = lmmcv.imresize(
                alpha, (new_w, new_h), interpolation='nearest')
            trimap = lmmcv.imresize(
                trimap, (new_w, new_h), interpolation='nearest')
            bg = lmmcv.imresize(bg, (new_w, new_h), interpolation='bicubic')
            h, w = new_h, new_w

        # resize to 1/4 to ignore small unknown patches
        small_trimap = lmmcv.imresize(
...
class CropAroundUnknown:
    ...
    def __init__(self,
        ...
        if not lmmcv.is_tuple_of(self.crop_sizes[0], int):
        ...
        elif lmmcv.is_list_of(interpolations,
...
class CropAroundFg:
    ...
    def __init__(self, keys, bd_ratio_range=(0.1, 0.4), test_mode=False):
        ...
        if (not lmmcv.is_tuple_of(bd_ratio_range, float)
