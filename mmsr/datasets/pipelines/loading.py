    ...
    def __call__(self, results):
        ...
        if self.convert_to is not None:
            if self.channel_order == 'bgr' and self.convert_to.lower() == 'y':
                img = lmmcv.bgr2ycbcr(img, y_only=True)
            elif self.channel_order == 'rgb':
                img = lmmcv.rgb2ycbcr(img, y_only=True)
            else:
...
class RandomLoadResizeBg:
    ...
    def __init__(self,
                 ...
                 **kwargs):
        self.bg_dir = bg_dir
        self.bg_list = list(lmmcv.scandir(bg_dir))
        self.io_backend = io_backend
        self.flag = flag
        self.channel_order = channel_order
        self.kwargs = kwargs
    ...
    def __call__(self, results):
        ...
        bg = lmmcv.imresize(img, (w, h), interpolation='bicubic')
        results['bg'] = bg
        return results
...
class LoadMask:
    ...
    def _get_random_mask_from_set(self):
        ...
        mask = lmmcv.imfrombytes(mask_bytes, flag=self.flag)  # HWC, BGR
        if mask.ndim == 2:
