...
class Resize:
    ...
    def __init__(self,
        ...
        elif lmmcv.is_tuple_of(scale, int):
    ...
    def _resize(self, img):
        if self.keep_ratio:
            img, self.scale_factor = lmmcv.imrescale(
                img,
                self.scale,
                return_scale=True,
            ...
            img, w_scale, h_scale = lmmcv.imresize(
                img,
                self.scale,
                return_scale=True,
                interpolation=self.interpolation,
...
class RandomRotation:
    ...
    def __init__(self, keys, degrees):
        ...
        elif not lmmcv.is_tuple_of(degrees, (int, float)):
    ...
    def __call__(self, results):
        ...
        for k in self.keys:
            results[k] = lmmcv.imrotate(results[k], angle)
            if results[k].ndim == 2:
            ...
            for key in self.keys:
                if isinstance(results[key], list):
                    for v in results[key]:
                        lmmcv.imflip_(v, self.direction)
                else:
                    lmmcv.imflip_(results[key], self.direction)
...
class RandomJitter:
    ...
    def __call__(self, results):
        ...
        fg = lmmcv.bgr2hsv(fg.astype(np.float32) / 255)
        # Hue noise
