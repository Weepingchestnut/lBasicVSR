...
class LIIFNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.imnet = build_backbone(imnet)
...
class LIIFRDN(LIIFNet):
    ...
    def gen_feature(self, x):
        ...
        sfe1 = self.sfe1(x)         # torch.Size([16, 64, 48, 48])
        sfe2 = self.sfe2(sfe1)      # torch.Size([16, 64, 48, 48])

        x = sfe2
        local_features = []
        for i in range(self.num_blocks):        # 16 个残差块
            x = self.rdbs[i](x)
