...
def psnr(img1, img2, crop_border=0, input_order='HWC', convert_to=None):
    ...
    if isinstance(convert_to, str) and convert_to.lower() == 'y':
        img1 = lmmcv.bgr2ycbcr(img1 / 255., y_only=True) * 255.
        img2 = lmmcv.bgr2ycbcr(img2 / 255., y_only=True) * 255.
...
def mae(img1, img2, crop_border=0, input_order='HWC', convert_to=None):
    ...
    if isinstance(convert_to, str) and convert_to.lower() == 'rgb2y':
        img1 = lmmcv.rgb2ycbcr(img1, y_only=True)
        img2 = lmmcv.rgb2ycbcr(img2, y_only=True)
    elif isinstance(convert_to, str) and convert_to.lower() == 'bgr2y':
        img1 = lmmcv.bgr2ycbcr(img1, y_only=True)
        img2 = lmmcv.bgr2ycbcr(img2, y_only=True)
    elif convert_to is not None:
        raise ValueError('Wrong color model. Supported values are '
...
def niqe(img, crop_border, input_order='HWC', convert_to='y'):
    ...
    if input_order != 'HW':
        ...
        if convert_to == 'y':
            img = lmmcv.bgr2ycbcr(img / 255., y_only=True) * 255.
        elif convert_to == 'gray':
            img = lmmcv.bgr2gray(img / 255., cv2.COLOR_BGR2GRAY) * 255.
        img = np.squeeze(img)
