...
def extract_subimages(opt):
    ...
    img_list = list(lmmcv.scandir(input_folder))
    img_list = [osp.join(input_folder, v) for v in img_list]

    prog_bar = lmmcv.ProgressBar(len(img_list))
...
def worker(path, opt):
    ...
    img = lmmcv.imread(path, flag='unchanged')
...
def prepare_keys_div2k(folder_path):
    ...
    img_path_list = sorted(
        list(lmmcv.scandir(folder_path, suffix='png', recursive=False)))
...
def make_lmdb(data_path,
    ...
    if multiprocessing_read:
        ...
        prog_bar = lmmcv.ProgressBar(len(img_path_list))
    ...
    img = lmmcv.imread(osp.join(data_path, img_path_list[0]), flag='unchanged')
